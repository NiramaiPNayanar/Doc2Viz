...
def strip_image_attributes(md_content):
    ...
    # Remove all variations of width/height attributes comprehensively
...
def clean_markdown_content(md_content, process_latex=True, process_underlines=True, save_json=False, cleaned_md_path=None):
    ...
    def complex_nested_fraction_handler(match):
        ...
        result = re.sub(r'\\(?:left|right|text|mathrm|rm|it|bf)(?:\{([^}]*)\}|([a-zA-Z]+))', r'\1\2', result)
    ...
    md_content = re.sub(r'\\mathrm\{([^}]+)\}', r'\1', md_content)
    ...
    # Remove standalone curly braces and malformed brace patterns
    # Remove isolated opening or closing braces
    md_content = re.sub(r'\s*\{\s*(?=\s|$)', ' ', md_content)  # Opening brace followed by space or end
    md_content = re.sub(r'(?<=\s)\s*\}\s*', ' ', md_content)  # Closing brace preceded by space
    md_content = re.sub(r'^\s*\}\s*', ' ', md_content, flags=re.MULTILINE)  # Closing brace at start of line
    # Remove empty braces
    md_content = re.sub(r'\{\s*\}', '', md_content)
    # Remove braces that only contain numbers or single characters
    md_content = re.sub(r'\{\s*(\d+|[A-Za-z])\s*\}', r'\1', md_content)
    # Remove braces around single words (but preserve LaTeX-style commands)
    md_content = re.sub(r'(?<!\\)\{\s*([A-Za-z]+)\s*\}(?![a-z])', r'\1', md_content)
